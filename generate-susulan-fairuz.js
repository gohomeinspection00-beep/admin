...
const data = {
  noRujukan: "SUSULAN/2026/FAIRUZ",

  namaPembeli: "MOHAMMAD FAIRUZ BIN ABU HANIPAH",
  alamatPengirim: [
    "No. 20, Jalan Bestari 66,",
    "Rini Homes 8, Taman Mutiara Rini,",
    "81300 Skudai,",
    "Johor Bahru, Johor.",
  ],
  telefonPembeli: "016-550 5869",
  emailPembeli: "mdfairuz.hanipah@gmail.com",
  ...
  jenisHartanah: "Rumah Teres 2 Tingkat (2-Storey Terrace, 2,200 kaki persegi)",
  namaProyek: "Rini Homes 8, Fasa 6L, Lot 197546",
  ...
  tarikhPemeriksaan1: "25 Julai 2026",
  tarikhSerahanLaporan: "29 Julai 2026",
  tarikhTamat30Hari: "29 Ogos 2026",

  tarikhNotis: "15 Ogos 2026",
  tarikhDeadline: "29 Ogos 2026",
  tempohNotis2: "15",

  kecacatan: [
    { tag: "176", lokasi: "Bedroom 2 — M&E", kecacatan: "Open ground pada soket 13A (Functionality issue — keselamatan pendawaian elektrik)", status: "Belum Dibaiki" },
    { tag: "266", lokasi: "RC Flat Roof — Floor", kecacatan: "Air bertakung pada permukaan papak lantai (Water stagnant on floor slab)", status: "Belum Dibaiki" },
    { tag: "274", lokasi: "Top Roof — Roof", kecacatan: "Genting bumbung pecah dan retak (Broken and crack on roof tiles — Major defects)", status: "Belum Dibaiki" },
    { tag: "275", lokasi: "Top Roof — Fixtures", kecacatan: "Tanda air bertakung di dalam gutter belakang (Sign of water stagnant inside gutter — Rear)", status: "Belum Dibaiki" },
    { tag: "276", lokasi: "Top Roof — Fixtures", kecacatan: "Tanda air bertakung di dalam gutter hadapan (Sign of water stagnant inside gutter — Front)", status: "Belum Dibaiki" },
    { tag: "277", lokasi: "Top Roof — Fixtures", kecacatan: "Air bertakung di dalam gutter (Water stagnant inside gutter)", status: "Belum Dibaiki" },
  ],

  kronologi: [
    { tarikh: "25 Julai 2026", peristiwa: "Pemeriksaan Kecacatan (Defect Inspection) dijalankan ke atas hartanah oleh Juruukur Bangunan berdaftar RISM" },
    { tarikh: "29 Julai 2026", peristiwa: "Laporan Pemeriksaan Kecacatan (123 muka surat) diserahkan secara serahan tangan ke Pejabat Projek pemaju dan dicop terima (\"RECEIVED\")" },
    { tarikh: "15 Ogos 2026", peristiwa: "Surat Susulan (Follow-Up) pembetulan kecacatan dikeluarkan" },
    { tarikh: "29 Ogos 2026", peristiwa: "Tamat tempoh 30 hari pembaikan di bawah Klausa 27(1) Jadual G — tarikh akhir pembaikan" },
  ],
...
bk();
const perkara1 = "Surat Susulan — Pembetulan Kecacatan (Follow-Up on Defect Rectification)";
...
para(
  `Saya, ${data.namaPembeli}, pemilik unit hartanah di alamat di atas, sebagaimana termaktub di dalam Perjanjian Jual Beli mengikut ${data.jenisSPA} (Projek: ${data.namaProyek}), telah menjalankan Pemeriksaan Kecacatan (Defect Inspection) pada ${data.tarikhPemeriksaan1} melalui Juruukur Bangunan berdaftar di bawah Royal Institution of Surveyors Malaysia (RISM). Laporan Pemeriksaan Kecacatan (Defect Inspection Report) sebanyak 123 muka surat telah diserahkan secara rasmi melalui serahan tangan kepada Pejabat Projek pihak tuan pada ${data.tarikhSerahanLaporan} dan telah dicop terima ("RECEIVED") oleh pihak tuan.`
);
y += 4;

numPara(2,
  `Klausa ${data.klausaPembaikan} ${data.jenisSPA} Perjanjian Jual Beli memperuntukkan bahawa pemaju hendaklah, atas kos dan belanjanya sendiri, membaiki dan memperbetulkan apa-apa kecacatan, pengecutan atau kerosakan lain yang menjejaskan hartanah tersebut dalam tempoh ${data.tempohDLP} bulan dari tarikh penyerahan milikan kosong (DLP — Defect Liability Period), dalam masa tiga puluh (30) hari selepas menerima notis bertulis daripada pembeli. Memandangkan laporan kecacatan telah diterima oleh pihak tuan pada ${data.tarikhSerahanLaporan}, tempoh tiga puluh (30) hari tersebut akan tamat pada ${data.tarikhTamat30Hari}.`
);
y += 4;

numPara(3,
  `Namun, sehingga tarikh surat ini dikeluarkan, tiada sebarang kerja pembaikan dimulakan atau dimaklumkan kepada saya oleh pihak tuan. Antara kecacatan yang dilaporkan dan masih belum dibaiki adalah seperti berikut:`
);
y += 5;
...
const fn = `*Senarai di atas bukanlah senarai penuh. Kecacatan lain adalah sebagaimana terkandung dalam Laporan Pemeriksaan Kecacatan (123 muka surat) yang telah diserahkan dan dicop terima oleh Pejabat Projek pihak tuan pada ${data.tarikhSerahanLaporan}, merangkumi antaranya Car Porch, Living Area, Dining, Kitchen, Working Area, semua bilik tidur, Water Tank dan Roof.`;
const fnL = doc.splitTextToSize(fn, cW);
for (const f of fnL) { checkBreak(6); doc.text(f, mL, y); y += 4.5; }
y += 5;

numPara(4,
  `Dengan ini, surat susulan (follow-up) ini dikeluarkan sebagai peringatan rasmi kepada pihak tuan supaya semua kerja pembaikan terhadap kecacatan yang dilaporkan disiapkan sepenuhnya selewat-lewatnya pada ${data.tarikhDeadline}, iaitu tarikh tamat tempoh tiga puluh (30) hari di bawah Klausa ${data.klausaPembaikan} ${data.jenisSPA}. Sekiranya pembaikan masih tidak disempurnakan selepas tarikh tersebut, Notis Pertama (First Notice) dan seterusnya Notis Akhir (Final Notice) akan dikeluarkan.`
);
y += 4;
...
bullet("Mengemukakan Notis Pertama (First Notice) dan seterusnya Notis Akhir (Final Notice) kepada pihak tuan dan pihak berkepentingan (stakeholders); dan");
...
numPara(7,
  `Sekiranya pihak tuan masih gagal mengambil tindakan selepas notis-notis berikutnya dikeluarkan, saya akan memfailkan tuntutan rasmi ke Tribunal Tuntutan Pembeli Rumah — TTPR (Homebuyer Claims Tribunal) di bawah Peraturan-peraturan Pemajuan Perumahan (Tribunal Tuntutan Pembeli Rumah) 2002 dan/atau apa-apa remedi lain yang diperuntukkan di bawah Akta Pemajuan Perumahan (Kawalan dan Pelesenan) 1966 (Akta 118) untuk mendapatkan perintah pembaikan atau pampasan yang sewajarnya.`
);
y += 4;

para("Saya berharap pihak tuan mengambil tindakan segera terhadap surat susulan ini. Atas kerjasama dan perhatian tuan diucapkan ribuan terima kasih.");
y += 4;
para("Sekian.");
y += 4;
...
function drawAkuanTerima(copyLabel) {
  ...
  doc.setFont("helvetica", "normal"); doc.setFontSize(SZ.BODY); bk();
  const akText = `Dengan ini diakui bahawa ${data.namaPemaju} ${data.noSyarikat} telah menerima Surat Susulan — Pembetulan Kecacatan (Follow-Up on Defect Rectification) bertarikh ${data.tarikhNotis} dengan rujukan ${data.noRujukan} daripada ${data.namaPembeli} berhubung hartanah di ${data.alamatHartanah}.`;
...
fs.writeFileSync("/home/user/admin/SURAT_SUSULAN_FAIRUZ.pdf", Buffer.from(out));
console.log("PDF generated: SURAT_SUSULAN_FAIRUZ.pdf");
