...
const data = {
  noRujukan: "MAKLUMAN-AR/2026/ZAFRAN",
  ...
  alamatPenerima: [
    "No. 2B-9, Jalan Kesidang 3/6,",
    "Melaka Mall, Off Jalan Tun Perak,",
    "75300 Melaka.",
  ],
  ...
  salinanKepada: [
    {
      nama: "SHANG HEIGHT REALTY SDN. BHD. — Pejabat Pengurusan",
      alamat: ["No. 19 & 19-1, Jalan Inang 1,", "Taman Paya Rumput Utama,", "76450 Melaka.", "Tel: 06-337 0871"],
    },
  ],
  // Surat ini ke alamat berdaftar; laporan penuh dihantar ke Pejabat Pengurusan
};
...
bk();
const perkara1 = "Makluman: Serahan Laporan Kecacatan Tambahan (3 Ruang) kepada Pejabat Pengurusan";
...
numPara(4,
  `Untuk makluman pihak tuan, had sistem dalaman aplikasi pemaju tidak boleh mengecilkan atau menangguhkan hak saya selaku pembeli di bawah Klausa ${data.klausaPembaikan} ${data.jenisSPA} Perjanjian Jual Beli, yang mewajibkan pemaju membaiki apa-apa kecacatan yang dimaklumkan secara bertulis dalam tempoh DLP.`
);
y += 4;

numPara(5,
  `Oleh yang demikian, surat ini adalah untuk memaklumkan secara rasmi kepada pihak tuan di alamat berdaftar bahawa Laporan Kecacatan Tambahan bagi tiga (3) ruang tersebut telah diserahkan kepada Pejabat Pengurusan pihak tuan di No. 19 & 19-1, Jalan Inang 1, Taman Paya Rumput Utama, 76450 Melaka melalui Pos Berdaftar Akuan Terima (AR) bertarikh ${data.tarikhSurat}, selaras dengan klausa Service of Documents (Klausa ${data.klausaSerahan} ${data.jenisSPA}). Serahan tersebut adalah dianggap sah dan diterima pakai sebagai serahan rasmi, dan tempoh pembaikan tiga puluh (30) hari bagi kecacatan di dalam laporan tambahan tersebut bermula dari tarikh serahan tersebut.`
);
y += 4;

numPara(6,
  "Saya berharap pihak tuan dapat menyelaraskan dengan Pejabat Pengurusan dan mengambil tindakan pembaikan terhadap semua kecacatan yang dilaporkan, termasuk item di dalam laporan tambahan tersebut, dalam tempoh yang ditetapkan. Sebarang pertanyaan boleh menghubungi saya di talian di atas."
);
y += 4;
...
doc.setFont("helvetica", "normal");
doc.text("1. Salinan surat serahan kepada Pejabat Pengurusan (Ruj: SERAHAN-AR/2026/ZAFRAN)", mL + 5, y); y += 4.5;
...
function drawAkuanTerima(copyLabel) {
  ...
  doc.setFont("helvetica", "normal"); doc.setFontSize(SZ.BODY); bk();
  const akText = `Dengan ini diakui bahawa ${data.namaPemaju} ${data.noSyarikat} telah menerima surat Makluman Serahan Laporan Kecacatan Tambahan (3 Ruang: ${data.ruangTertinggal}) bertarikh ${data.tarikhSurat} dengan rujukan ${data.noRujukan} beserta lampirannya daripada ${data.namaPembeli} berhubung hartanah di ${data.alamatHartanah}.`;
...
fs.writeFileSync("/home/user/admin/SURAT_MAKLUMAN_ZAFRAN_KESIDANG.pdf", Buffer.from(out));
console.log("PDF generated: SURAT_MAKLUMAN_ZAFRAN_KESIDANG.pdf");
